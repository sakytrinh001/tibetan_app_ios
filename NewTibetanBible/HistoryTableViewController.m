...
- (void)viewDidLoad {
    ...
    self.title = @"བྱུང་ཟིན།";
    allVerseId = [self.listHistory allKeys];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        ...
        if (verse) {
            ...
            cell.detailTextLabel.text = [[self.listHistory objectForKey:verse] stringByReplacingOccurrencesOfString:@"&nbsp; &nbsp;" withString:@""];
        }
        return cell;
    }
    return nil;
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 1) {
        if (editingStyle == UITableViewCellEditingStyleDelete) {
            ...
            [self.listHistory removeObjectForKey:verse];
            allVerseId = [self.listHistory allKeys];
            [tableView deleteRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationFade];
            // Save favorite array change
            NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:self.listHistory forKey:@"HistoryKey"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 1) {
        NSString *verse = [allVerseId objectAtIndex:indexPath.row];
        [self.delegate scrollToHistory:verse];
    }
}
